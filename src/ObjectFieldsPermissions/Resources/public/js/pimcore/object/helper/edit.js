    ...
    getRecursiveLayout: function (l, properties, noteditable, context, skipLayoutChildren, onlyLayoutChildren, dataProvider, disableLazyRendering) {
        ...
        if (l.datatype == "layout") {
                ...
                if (l.childs.length > 0) {
                    ...
                    for (var i = 0; i < l.childs.length; i++) {
                        ...
                        if(l.fieldtype =='tabpanel' && !disableLazyRendering) {
                            tmpItems = this.getRecursiveLayout(childConfig, properties, noteditable, context, true, false, dataProvider, disableLazyRendering);
                            if (tmpItems) {
                                ...
                                tmpItems['listeners']['afterrender'] = function (childConfig, panel) {
                                    if (!panel.__tabpanel_initialized) {
                                        var children = this.getRecursiveLayout(childConfig, properties, noteditable, context, false, true, dataProvider, disableLazyRendering);
                                        panel.add(children);
                                        panel.updateLayout();
                            ...
                            tmpItems = this.getRecursiveLayout(childConfig, properties, noteditable, context, false, false, dataProvider, disableLazyRendering);
                        }
            ...
            if (pimcore.object.tags[l.fieldtype]) {
                ...
                if(l.mandatory || (properties && properties[l.name] && properties[l.name].mandatory)) {
                ...
                if (l.noteditable || noteditable || (properties && properties[l.name] && properties[l.name].noteditable)) {
                    field.fieldConfig.noteditable = true;
